package com.codecatalyst.component.behavior.ui
{
    ...
    public class Popup extends AbstractBehavior implements IBehavior
    {
        ...
        /**
         * Enable content to be draggable
         */
         ...
         *
         */
        public function Popup(renderer:IFactory = null, parent:DisplayObject = null, params:Object = null)
        {
         ...
         */
        public function show(map:Object = null):IFlexDisplayObject
        {
            ...
            // Do have a `map` of key/value pairs
            // to assign during the show() action ?
            if (map != null)
            {
                for (var key:* in map)
                {
        ...
        protected function onContentReady(event:FlexEvent):void
        {
            ...
            content.addEventListener(MouseEvent.MOUSE_DOWN, onStartDrag, false, 0, true);
        ...
        protected function onCloseContent(event:CloseEvent = null):void
        {
            ...
            if (content.stage)
            {
                ...
                content.removeEventListener(MouseEvent.MOUSE_DOWN, onStartDrag);
            }
        ...
        /**
         *
         * If mouseDown target phase (during drilldown / before bubbling)
         * if off-content, then autoClose if needed.
         *
         */
        ...
        /**
         * If mouseDown bubble phase, then see if the content is the target
         * and conditionally start dragging...
         *
         */
        protected function onStartDrag(event:MouseEvent):void
        {
            ...
            {
                content.removeEventListener(MouseEvent.MOUSE_UP, onStopDrag);
                content.stopDrag();
            }

            if (draggable && (event.target == content))
            {
                content.addEventListener(MouseEvent.MOUSE_UP, onStopDrag);
                content.startDrag();
            }
        }
        ...
        {
            var results:IEffect = (effect is IFactory) ? IFactory(effect).newInstance() as IEffect : (effect is Class) ? new effect() as IEffect : effect as IEffect;
        ...
        {
            return (content && (content is UIComponent) && UIComponent(content).initialized);
        }
        ...
        private function mouseIsOver(target:DisplayObject):Boolean
        {
            ...
            return target.hitTestPoint(target.stage.mouseX, target.stage.mouseY, false);
        }
...
class Anchors
{
    ...
    private function applyAnchors(bounds:Rectangle, child:IUIComponent = null):void
    {
        ...
        w = (!isNaN(left) && !isNaN(right)) ? Math.max(availableWidth - left - right, child.minWidth) : (!isNaN(child.percentWidth)) ? inBounds(child.percentWidth / 100 * availableWidth, child.minWidth, child.maxWidth) : child.getExplicitOrMeasuredWidth();

        //The height of the region which the control will live in. 

        h = (!isNaN(top) && !isNaN(bottom)) ? Math.max(availableHeight - top - bottom, child.minHeight) : (!isNaN(child.percentHeight)) ? inBounds(child.percentHeight / 100 * availableHeight, child.minHeight, child.maxHeight) : child.getExplicitOrMeasuredHeight();

        // The left, right, and horizontalCenter styles affect the child's x and/or its actual width.

        x = !isNaN(horizontalCenter) ? Math.round((availableWidth - w) / 2 + horizontalCenter) : !isNaN(left) ? left : !isNaN(right) ? availableWidth - right - w : child.y;

        // The top, bottom, verticalCenter and baseline styles affect the child's y and/or its actual height.

        y = !isNaN(verticalCenter) ? Math.round((availableHeight - h) / 2 + verticalCenter) : !isNaN(top) ? top : !isNaN(bottom) ? availableHeight - bottom - h : child.x;
    ...
    {
        return (a < min) ? min : (a > max) ? max : Math.floor(a);
    }
